...
from fd_common.utils            import fd_format_elapsed_time
...
def fd_get_cali_file(file_type: int, file_directory, t_start = 0, cam_index = 0, with_sec = True) -> str:
    ...
    filename_map = {
        conf._calibration_no_audio    : f"{conf._calibration_multi_prefix}_{base_name}_{cam_index:02}.mp4",
        conf._calibration_with_audio  : f"{conf._calibration_multi_prefix}_{base_name}.{conf._output_shared_audio_type}",
    }
    filename = filename_map.get(file_type, f"output_{file_type:02X}.mp4")
    output_file = os.path.join("R:\\", filename)
...
class CalibrationVideo:
    ...
    _NVENC_START_SEM: threading.Semaphore | None = None    

    def __init__(self):

        # 환경변수 우선, 없으면 conf 별칭/원본 경로, 그래도 없으면 기본값
        init_default = getattr(conf, "_gpu_session_init_cnt", 6)
        max_default  = getattr(conf, "_gpu_session_max_cnt", 12)

        init_cnt = int(os.environ.get("FD_NVENC_INIT_CONCURRENCY", init_default))
        self._NVENC_START_SEM = threading.Semaphore(init_cnt)
        
        # concurrent NVENC session slots (default by env or conf)
        slots = int(os.environ.get("FD_NVENC_MAX_SLOTS", max_default))        
        self.locker = NVEncLocker(slots)
    ...
    @classmethod
    def _ensure_nvenc_sems(cls) -> None:
        """환경변수/설정값을 읽어 NVENC 세마포어를 지연 초기화."""
        if cls._NVENC_START_SEM is not None and cls._NVENC_SLOTS_SEM is not None:
            return

        # 설정 우선순위: 환경변수 → conf(Runtime.GPUConfig) → 하드코드 기본값
        init_conc = int(os.environ.get(
            "FD_NVENC_INIT_CONCURRENCY",
            getattr(conf, "_gpu_session_init_cnt", 6)
        ))
        max_slots = int(os.environ.get(
            "FD_NVENC_MAX_SLOTS",
            getattr(conf, "_gpu_session_max_cnt", 12)
        ))

        # 음수/0 방어
        init_conc = max(1, init_conc)
        max_slots = max(1, max_slots)

        cls._NVENC_START_SEM = threading.Semaphore(init_conc)
        cls._NVENC_SLOTS_SEM = threading.BoundedSemaphore(max_slots)
    ...
    def _concat_to_temp_copy_no_check(self, files) -> str | None:
        ...
        try:
            ...
            ff = getattr(sys.modules.get("__main__"), "conf", None)._path_ffmpeg if hasattr(sys.modules.get("__main__"), "conf") else "ffmpeg"
            if not ff:
                ff = "ffmpeg"
    ...
    def _nvenc_probe_loop(self, width, height, *, vcodec="h264", ffmpeg_candidates=None,
        ...
        if ffmpeg_candidates is None:
            ffmpeg_candidates = [
                getattr(sys.modules.get("__main__"), "conf", None)._path_ffmpeg if hasattr(sys.modules.get("__main__"), "conf") else None,
    ...
    def run(self,
        ...
        def _open_pipe(ffbin):
            ...
            if not start_sem_held:                
                CalibrationVideo._ensure_nvenc_sems()  # ★ 추가
...
def process_video_parts(file_directory, camera_ip_class, camera_ip, file_type):
    ...
    match file_type:
        ...
        case conf._calibration_no_audio:
...
def fd_set_mem_file_calis_audio(file_path, cam_ip_class, cam_ip, channel, tg_index, start_time, start_frame, end_time, end_frame):    
    ...
    conf._thread_file_calibration[tg_index][0] = threading.Thread(target=process_audio_parts, args=(file_path, tg_index, cam_ip_class, cam_ip, conf._calibration_no_audio, start_time, start_frame, end_time, end_frame))
    conf._thread_file_calibration[tg_index][0].start()
    
    #    process_audio_parts(file_path, tg_index, cam_ip_class, cam_ip, conf._calibration_no_audio, start_time, start_frame, end_time, end_frame)
...
def fd_set_mem_file_calis(file_path, cam_ip_class, cam_ip, channel, tg_index, cam_index, start_time, start_frame, end_time, end_frame, adjust_set, cam_audio):
    ...
    conf._thread_file_calibration[tg_index][cam_index+1] = threading.Thread(target=process_video_parts_pipe, args=(file_path, tg_index, cam_index, cam_ip_class, cam_ip, conf._calibration_no_audio, start_time, start_frame, end_time, end_frame, channel, adjust_info, shared_audio))
    conf._thread_file_calibration[tg_index][cam_index+1].start()

    # non thread
    #process_video_parts_new(file_path, tg_index, cam_index, cam_ip_class, cam_ip, conf._calibration_no_audio, start_time, start_frame, end_time, end_frame, channel, adjust_info, shared_audio)
...
def fd_set_mem_file_multiline():
    ...
    conf._clean_file_list       = fd_get_clean_file_name(conf._path_local_temp)
...
def fd_combine_calibrated_output(camera_groups):    
    ...
    for camera in camera_groups:
        ...
        for idx, start_time in enumerate(times):
            file_name = fd_get_cali_file(conf._calibration_no_audio, file_directory, start_time, channel)
...
def process_audio_parts(file_directory, tg_index, camera_ip_class, camera_ip,
    ...
    # ───────── helpers ─────────
    def _run(cmd, check=True, debug_tag=None):
        # 실행 파일 이름을 풀패스로 대체
        if isinstance(cmd, (list, tuple)) and cmd:
            head = str(cmd[0]).lower()
            if head.endswith("ffmpeg") or head.endswith("ffmpeg.exe"):
                cmd = [ _ffmpeg_bin(), *cmd[1:] ]
            elif head.endswith("ffprobe") or head.endswith("ffprobe.exe"):
                cmd = [ _ffprobe_bin(), *cmd[1:] ]

        p = subprocess.run(cmd, text=True, capture_output=True)
        if check and p.returncode != 0:
            raise RuntimeError(f"{debug_tag or ''} ff cmd failed: {p.stderr}")
        return p
    ...
    def _ffmpeg_bin() -> str:
        """
        ffmpeg 실행 파일의 절대 경로를 반환.
        우선순위: conf.Paths.ffmpeg_path → conf._path_ffmpeg → PATH(환경변수) → 기본 위치 추정
        """
        # 1) conf 우선
        cand = getattr(conf, "ffmpeg_path", None) or getattr(conf, "_path_ffmpeg", None)
        if cand:
            p = Path(cand)
            # ffmpeg.exe 파일까지 지정되어 있지 않고 디렉터리만 온 경우도 처리
            if p.is_dir():
                exe = p / "ffmpeg.exe"
            else:
                exe = p
        ...
        # 3) 프로젝트 내 기본 위치(필요시 경로 조정)
        # 예: C:/4DReplay/AId/library/ffmpeg/ffmpeg.exe
        root = Path(__file__).resolve().parents[1]  # aid/ 기준 상위
        exe = root / "library" / "ffmpeg" / "ffmpeg.exe"
        if exe.exists():
            return str(exe)

        raise FileNotFoundError(
            "ffmpeg 실행 파일을 찾을 수 없습니다. conf.Paths.ffmpeg_path를 확인하세요."
        )

    def _ffprobe_bin() -> str:
        # conf.Paths.ffmpeg_path에 ffprobe.exe도 함께 있다고 가정
        cand = getattr(conf, "ffmpeg_path", None) or getattr(conf, "_path_ffmpeg", None)
        if cand:
            p = Path(cand)
            exe = p / "ffprobe.exe" if p.is_dir() else Path(str(p).replace("ffmpeg.exe", "ffprobe.exe"))
            if exe.exists():
                return str(exe)
        ...
        exe = root / "library" / "ffmpeg" / "ffprobe.exe"
        if exe.exists():
            return str(exe)

        raise FileNotFoundError(
            "ffprobe 실행 파일을 찾을 수 없습니다. conf.Paths.ffmpeg_path를 확인하세요."
        )
        
    def _has_audio_stream(path: str) -> bool:
        # 옵션 1: codec_type 열만 출력 → audio 포함 여부
        p = _run(
            ["ffprobe", "-v", "error",
