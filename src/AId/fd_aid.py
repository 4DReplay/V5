...
def fd_create_analysis_file(type_target, folder_input, folder_output, camera_ip_class, camera_ip, start_time, end_time, selected_moment_sec, selected_moment_frm, fps, zoom_ratio): 
    ...
    match type_target:        
        ...
        # ─────────────────────────────────────────────────────────────────────────────#
        case conf._type_target_golfer_2ch_LND_RH | \
             conf._type_target_golfer_3ch_LND_RH | \
             conf._type_target_golfer_3ch_POR_RH :
            conf._swing_right_hand = True
            if(type_target == conf._type_target_golfer_3ch_POR_RH):
        ...
        case conf._type_target_golfer_2ch_LND_LH | \
            conf._type_target_golfer_3ch_LND_LH  | \
            conf._type_target_golfer_3ch_POR_LH : 
            if(type_target == conf._type_target_golfer_3ch_POR_LH):
                conf._image_portrait = True
            else:
                conf._image_portrait = False     
    ...
    match type_target:        
        ...
        # 2025-04-29
        # golf
        case conf._type_target_golfer_2ch_LND_RH | conf._type_target_golfer_2ch_LND_LH | \
             conf._type_target_golfer_3ch_LND_RH | conf._type_target_golfer_3ch_LND_LH | \
             conf._type_target_golfer_3ch_POR_RH | conf._type_target_golfer_3ch_POR_LH :  
    ...
    # debug
    # play output video
    if not conf._test_mode and not conf._live_player:
...
def check_verified_calibration_input_data(Cameras, Markers, AdjustData, prefix, output_path, logo_path, resolution, codec, fps, bitrate, gop, output_mode):
    ...
    expected_time = conf._time_video_length * conf._calibrate_camera_count * conf._time_cali_time_per_camera
