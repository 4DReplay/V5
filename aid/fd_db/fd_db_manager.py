...
class BaseballDB:
    def __init__(self, db_file):
        ...
        try:
            ...
            print(f"✅ Database connection established: {self.db_file}")

            if os.path.exists(self.db_file):
                print(f"✅ Database file exists: {self.db_file}")
        ...
        for raw_table, raw_sql in raw_table_creates.items():
            ...
            if not self.cursor.fetchone():
                try:
                    ...
                    print(f"✅ Raw 테이블 생성됨: {raw_table}")
                except sqlite3.Error as e:
                    print(f"❌ Raw 테이블 생성 실패: {raw_table}, 오류: {e}")
            ...
            for column_name, column_type in columns.items():
                if column_name not in existing_columns:
                    ...
                    try:
                        ...
                        print(f"✅ 컬럼 추가됨: {table_name}.{column_name} ({column_type})")
                    except sqlite3.OperationalError as e:
                        print(f"❌ ALTER TABLE 실패: {e}")
    ...
    def insert_raw_data(self, json_data):
        ...
        if kind == "Pitch":
            ...
            print(f"❌ 알 수 없는 Kind 유형: {kind}")
            return

        try:
            self.cursor.execute(raw_query, raw_values)
            self.conn.commit()
            print(f"📥 Raw structured data inserted into {kind.lower()}_raw_data for PlayId {json_data.get('PlayId')}.")
        except Exception as e:
            print(f"❌ Raw structured data 삽입 오류: {e}")
            print("📛 VALUES 개수:", len(raw_values))
            print("📛 SQL placeholders 개수:", raw_query.count("?"))
    ...
    def insert_data(self, json_data, tracking_video_path , tracking_data_path):
        ...
        if not isinstance(json_data, dict):
            print(f"❌ 잘못된 JSON 데이터 형식: {type(json_data)}")
            return

        required_keys = ["PlayId", "Kind", "Time", "data"]
        for key in required_keys:
            if key not in json_data:
                print(f"❌ JSON 데이터에 {key} 키가 없습니다.")
                return
        ...
        if json_data["Kind"] == "Pitch":
            ...
            print(f"❌ 알 수 없는 Kind 유형: {json_data['Kind']}")
            return

        try:
            self.cursor.execute(query, values)
            self.conn.commit()
            print(f"✅ Data inserted successfully for PlayId {play_id}.")
        except Exception as e:
            print(f"❌ 데이터 삽입 오류: {e}")
    ...
    def count_pitches(self, play_id=None):
        ...
        print(f"📊 Total pitches data count: {count} (PlayId: {play_id})")
        return count
    ...
    def count_hits(self, play_id=None):
        ...
        self.cursor.execute(query, (play_id,) if play_id else ())
        count = self.cursor.fetchone()[0]
        print(f"📊 Total hits data count: {count} (PlayId: {play_id})")
        return count
    ...
    def count_raw_pitches(self, play_id=None):
        ...
        self.cursor.execute(query, (play_id,) if play_id else ())
        count = self.cursor.fetchone()[0]
        print(f"📊 Total pitches raw data count: {count} (PlayId: {play_id})")
        return count
    ...
    def count_raw_hits(self, play_id=None):
        ...
        self.cursor.execute(query, (play_id,) if play_id else ())
        count = self.cursor.fetchone()[0]
        print(f"📊 Total hits raw data count: {count} (PlayId: {play_id})")
        return count
    ...
    def save_pitch_data_to_file(self, pitch_data: dict, filename: str, output_dir: str = "./output"):
        ...
        print(f"Pitch data saved to {filepath}")
