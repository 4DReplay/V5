...
set "AID_DIR=%APP_ROOT%\daemon\AId"
...
rem ── 부트 로깅 ───────────────────────────────────────────────────────────────
set "BOOT_LOG=%LOG_DIR%\_aid_boot.log"
(
...
rem ── Python 메인 실행 ───────────────────────────────────────────────────────
echo ==== %DATE% %TIME% :: launching aid_main.py ====>> "%BOOT_LOG%"
"%PYTHON_EXE%" -u "%SRC_DIR%\AId\aid_main.py"
set "RC=%ERRORLEVEL%"
echo ==== %DATE% %TIME% :: aid_main.py exit rc=%RC% ====>> "%BOOT_LOG%"

rem ── 리턴코드 기록 ──────────────────────────────────────────────────────────
echo %RC%> "%FD_LOG_DIR%\_aid.rc"
endlocal & exit /b %RC%
