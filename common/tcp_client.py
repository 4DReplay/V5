...
class TCPClient:
    ...
    def connect(self, ip: str, port: int, callback=None, timeout: int=3) -> bool:
        try:
            ...
            if e.errno == errno.EISCONN:
                logger.debug(f'Already connected [{self.name}]')
                return True
            else:
                logger.error(f'Failed to create socket [{self.name}] : {e.strerror}')
                return False

        self.cb = callback
        self.recv_th = threading.Thread(target=self.recv_thread_func)
        self.recv_th.start()
        logger.debug(f'Connection successful [{self.name}]')
        return True
    ...
    def recv_thread_func(self):
        ...
        while not self.stop_evt.is_set():
            try:
                ...
                logger.error(f'Recv error [{self.name}] : {e.strerror}')
                break
        self.sock.close()
        logger.debug(f'Finish recv thread')
    ...
    def send_msg(self, msg) -> bool:
        if not self.is_connected():
            logger.error(f'Fail to send message: Disconnected:\n{msg}')
            return False
        ...
        with self.lock:
            ...
            try:
                ...
                if send_size != pkt_size:
                    logger.error(f'Send error.. size: {send_size} / {pkt_size}')
                    return False
